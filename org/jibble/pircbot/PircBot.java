 ...
 * framework.
 * <p>
 * NOTICE: This version of PircBot, informally labeled 1.4.5, is a project fork
 * customized to fit the IMP project.  The code has been largely reworked and optimized,
 * with many features added.  The base framework and much of the backbone is still
 * credited to Paul James Mutton, with changes and further development throughout
 * the package by Tom Frost.
 * 
 * @author  Paul James Mutton,
 *          <a href="http://www.jibble.org/">http://www.jibble.org/</a> 
 *          Re-worked and expanded by Tom Frost,
 *          <a href="http://blog.frosteddesign.com">http://blog.frosteddesign.com</a>
 * @version    1.4.5
 */
public abstract class PircBot implements ReplyConstants {
     ...
     */
    public static final String VERSION = "1.4.5-ed";
    
    
    private static final int OP_ADD = 1;		// +o, @
    private static final int OP_REMOVE = 2;
    private static final int VOICE_ADD = 3;		// +v, +
    private static final int VOICE_REMOVE = 4;
    private static final int HOP_ADD = 5;		// +h, %
    private static final int HOP_REMOVE = 6;
    private static final int ADMIN_ADD = 7;		// +a, &
    private static final int ADMIN_REMOVE = 8;
    private static final int OWNER_ADD = 9;		// +q, ~
        ...
        while ((line = breader.readLine()) != null) {
            ...
            if (secondSpace >= 0) {
                ...
                if (code.equals("004")) {
                    ...
                    // we tried to perform a kick or something that we are not allowed to do
                    break;
                }
    ...
    /**
     * Grants half-operator privilidges to a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're half-opping the user on.
     * @param nick The nick of the user we are half-opping.
     */
    ...
    /**
     * Removes half-operator privilidges from a user on a channel.
     * Successful use of this method may require the bot to have operator
     * status itself.
     * 
     * @param channel The channel we're de-half-opping the user on.
     * @param nick The nick of the user we are de-half-opping.
     */
    ...
    private final void processServerResponse(int code, String response) {
        ...
        if (code == RPL_LIST) {
        	...
        	if (data.length >= 7)
        		this.addUser(data[1], new User(data[6].substring(1), data[5], data[2], data[3]));
        }
        else if (code == RPL_ENDOFWHO) {
        	String channel = response.substring(response.indexOf(' ') + 1, response.indexOf(" :"));
        	this.onUserList(channel, this.getUsers(channel));
        	this.sendRawLine("MODE " + channel + " b");  // Get the ban list..
        }
        else if (code == RPL_BANLIST) {
        	String[] data = response.split(" ");
        	if (data.length >= 2) {
        		if (!banLists.containsKey(data[1]))
        			banLists.put(data[1], new ArrayList<String>());
        		if (!banLists.get(data[1]).contains(data[2]))
        			banLists.get(data[1]).add(data[2]);
        	}
        }
        else if (code == RPL_ENDOFBANLIST) {
        	String[] data = response.split(" ", 3);
        	if (banLists.containsKey(data[1]))
        		this.onBanList(data[1], banLists.get(data[1]).toArray(new String[] {}));
        	else
        		banLists.put(data[1], new ArrayList<String>());
        }
    ...
    /**
     * This method is called when we receive a ban list from the server
     * after joining a channel.
     *  <p>
     * Shortly after joining a channel, the PircBot client requests a list of all
     * banmasks in that channel. The PircBot collects this information and
     * calls this method as soon as it has the full list.
     *  <p>
     * At a later time, you may call the getBanlist method to obtain an
     * up to date list of the ban masks in the channel.
     *  <p>
     * The implementation of this method in the PircBot abstract class
     * performs no actions and may be overridden as required.
     * 
     ...
     * 
     * @param channel The name of the channel.
     * @param banlist An array of the banned user masks.
     */
    protected void onBanList(String channel, String[] banlist) {}
            ...
            for (int i = 0; i < params[0].length(); i++) {
                ...
                if (atPos == '+' || atPos == '-') {
                   ...
                   p++; 
                    ...
                    if (pn == '+') {
                    	if (!banLists.containsKey(channel))
                    		banLists.put(channel, new ArrayList<String>());
                    	if (!banLists.get(channel).contains(params[p]))
                    		banLists.get(channel).add(params[p]);
                    	...
                    	if (!banLists.containsKey(channel))
                    		banLists.put(channel, new ArrayList<String>());
                    	if (banLists.get(channel).contains(params[p]))
                    		banLists.get(channel).remove(params[p]);
    ...
    /**
     * Called when a user (possibly us) gets removed from owner status for a channel.
     *  <p>
     ...
     * @param recipient The nick of the user that got 'deownered'.
     */
     ...
     * @param sourceLogin The login of the user that performed the mode change.
     * @param sourceHostname The hostname of the user that performed the mode change.
     * @param recipient The nick of the user that got 'ownered'.
     */
    ...
    /**
     * Called when a user (possibly us) is removed from admin status for a channel.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the PircBot class.
     *  <p>
     ...
     * @param sourceLogin The login of the user that performed the mode change.
     * @param sourceHostname The hostname of the user that performed the mode change.
     * @param recipient The nick of the user that got 'deadmined'.
     */
    ...
    /**
     * Called when a user (possibly us) gets granted admin status for a channel.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the PircBot class.
     *  <p>
     ...
     * @param sourceLogin The login of the user that performed the mode change.
     * @param sourceHostname The hostname of the user that performed the mode change.
     * @param recipient The nick of the user that got 'admined'.
     */
    ...
    public final User[] getUsers(String channel) {
        ...
        synchronized (_channels) {
            Hashtable<String, User> users = _channels.get(channel);
            if (users != null) {
    ...
    /**
     * Returns a Hashtable of all users in the specified channel.
     * The keys in the table are the users' lowercase nicknames, while the
     * values are the actual User objects that represent them.
     *  <p>
     * There are some important things to note about this method:-
     * <ul>
     *  <li>This method may not return a full list of users if you call it
     ...
     * @param channel The name of the channel to list.
     * 
     * @return A Hashtable<String, User> of User objects. Returns null if the bot is
     * 		not in the channel.
     * 
     * @see #onUserList(String,User[]) onUserList
     */
    public final Hashtable<String, User> getUserTable(String channel) {
    	return _channels.get(channel.toLowerCase());
    }
    
    
    /**
     * Returns an array of all ban masks set in the specified channel.
     *  <p>
     * There are some important things to note about this method:-
     * <ul>
     *  <li>This method may not return a full list of ban masks if you call it
     *      before the complete ban list has arrived from the IRC server.
     *  </li>
     *  <li>If you wish to find out the entire channel's ban list as soon
     *      as you join it, then you should override the onBanList method
     *      instead of calling this method, as the onBanList method is only
     *      called as soon as the full ban list has been received.
     *  </li>
     *  <li>This method will return immediately, as it does not require any
     *      interaction with the IRC server.
     *  </li>
     *  <li>The bot must be in a channel to be able to know which masks are
     *      banned in it.
     *  </li>
     * </ul>
     * 
     ...
     * @param channel The name of the channel to list.
     * 
     * @return An array of ban masks. This array is empty if we are not
     *         in the channel.
     * 
     * @see #onUserList(String,User[]) onUserList
     */
    public final String[] getBanList(String channel) {
    	if (banLists.containsKey(channel))
    		return banLists.get(channel).toArray(new String[] {});
    	else
    		return null;
    }
    ...
    private final void renameUser(String oldNick, String newNick) {
        synchronized (_channels) {
            Enumeration<String> enumeration = _channels.keys();
            while (enumeration.hasMoreElements()) {
                Hashtable<String, User> users = _channels.get(enumeration.nextElement());
                if (users.containsKey(oldNick.toLowerCase())) {
                	User temp = users.remove(oldNick.toLowerCase());
    ...
    private final void removeAllChannels() {
        synchronized(_channels) {
            _channels = new Hashtable<String, Hashtable<String, User>>();
        }
    }
    ...
    // A Hashtable of channels that points to a Hashtable of
    // User objects (used to remember which users are in which channels).
    private Hashtable<String, Hashtable<String, User>> _channels = new Hashtable<String, Hashtable<String, User>>();
    ...
    // A Hashtable to store ArrayLists of banned users.
    private Hashtable<String, ArrayList<String>> banLists = new Hashtable<String, ArrayList<String>>();
